...
def request_popular(lim: int = 50) -> str:
    '''
    ...
    return safe_get(baseurl + "?" + urllib.parse.urlencode(params))
...
def get_games_info(games: list[str]) -> str:
    '''
...
def get_historic_low(games: list[str]) -> json:
    '''
...
def load_historical_low(games: list[str]) -> dict:
    '''
    ...
    for i in range(0, len(games), 5):
        ...
        time.sleep(0.1)
        results = json.loads(get_historic_low(games[i:i + 5]))['data']
        temp.update(results)
    # Handle the remaining game in the remainder
    if i < len(games):
        time.sleep(0.1)
        results = json.loads(get_historic_low(games[i:len(games)]))['data']
        temp.update(results)
    return temp
...
if __name__ == "__main__":
    ...
    most_popular = json.loads(request_popular(50))
    most_popular_plains = parse_popular(most_popular)

    # Get basic info about those games
    popular_games_info = json.loads(get_games_info(most_popular_plains))['data']
    ...
    popular_games_lowest = load_historical_low(most_popular_plains)
    # Output a csv file containing review score information about those games
